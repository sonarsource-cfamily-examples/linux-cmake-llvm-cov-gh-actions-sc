...
. Run `llvm-cov show --show-branches=count` on all the test executable files (or all object files you are interested in). `--show-branches=count` should not be used for SonarQube versions earlier than `10.0`.
